...
FILE_TEST_OPS="operatii/operatii%d.in"
...
# Points:
# Teste  1 - 10	=> instructiuni simple 		(40pt)
# Teste 11 - 15	=> instructiuni complexe 	(45pt)
# Teste 16 - 18	=> teste mai mari 		(15pt)
# Bonus  1 - 3	=> bonus mai simplu		(15pt)
# Bonus  4 - 6	=> bonus mai complex		(15pt)
...
function run_test {
	...
	./$EXEC_NAME $2 $3 $4 > /dev/null 2>&1 | cat
...
function run_tests {
	...
	do
		...
		printf -v file_ops $3 $i
		printf -v file_out $4 $i
		printf -v file_ref $5 $i
		run_test $i $file_date $file_ops $file_out $file_ref $6
...
function run_normal_tests {
	...
	echo "[TESTE NORMALE]"
	xxx="CACAT"
	run_tests $NUM_TESTS $FILE_TEST_DAT $FILE_TEST_OPS $FILE_TEST_OUT $FILE_TEST_REF "$POINTS_TEST" 0
...
function run_bonus_tests {
	echo $DELIM
	echo "[TESTE BONUS]"
	run_tests $NUM_BONUS $FILE_BONUS_DAT $FILE_BONUS_OPS $FILE_BONUS_OUT $FILE_BONUS_REF "$POINTS_BONUS" 1
